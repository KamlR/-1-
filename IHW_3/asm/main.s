...
main:
	...
	mov	DWORD PTR -68[rbp], edi # на стек положили argc (кол-во аргументов командной строки)
	...
	pxor	xmm0, xmm0
	cvtsi2sd	xmm0, eax
	movsd	QWORD PTR -24[rbp], xmm0 # на стек по адресу rbp[-24] положили number 
	...
	pxor	xmm0, xmm0
	cvtsi2sd	xmm0, eax
	movsd	QWORD PTR -32[rbp], xmm0 # на стек по адресу rbp[-32] положили del
	movsd	xmm0, QWORD PTR -24[rbp]
	divsd	xmm0, QWORD PTR -32[rbp] # поделили number на del
	movsd	QWORD PTR accuracy[rip], xmm0 # результат деления положили в глоб. переменную, отвечающую за точность вычислений
	cmp	DWORD PTR -68[rbp], 3 # сравниваем argc с 3
	...
	jmp	.L3
.L2:
	cmp	DWORD PTR -68[rbp], 4 # сравниваем argc с 4
