...
FILE *output;
pthread_mutex_t mutex_single;  // mutex for checking single rooms
pthread_mutex_t mutex_double;  // mutex for checking double rooms
pthread_mutex_t mutex_removing;// mutex for remove guests
...
void *settling(void *indexes) {
    ...
    for (int i = index_start; i < index_end; ++i) {
        ...
        if (i % 2 == 0) {
            ...
            int flag = 0;
            pthread_mutex_lock(&mutex_single);// lock the code because there is a record of data into array
            ...
            pthread_mutex_unlock(&mutex_single);// unlock the section because we stop working with  single rooms
            if (flag == 0) {
            ...
            int flag = 0;
            pthread_mutex_lock(&mutex_double);// lock the section because of working with double room
            ...
            pthread_mutex_unlock(&mutex_double);// unlock the section because we stop working with double room
        ...
        if (i % 3 == 0) {                       // guest eviction
            pthread_mutex_lock(&mutex_removing);// lock the section because we will work with common data
            removing();
            pthread_mutex_unlock(&mutex_removing);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    pthread_mutex_init(&mutex_single, NULL);
    pthread_mutex_init(&mutex_double, NULL);
    pthread_mutex_init(&mutex_removing, NULL);
