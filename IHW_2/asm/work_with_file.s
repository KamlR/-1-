...
work_with_file:
	...
	mov	DWORD PTR -4[rbp], 0 # переменной count_vowels в си присваиваем 0
	mov	DWORD PTR -8[rbp], 0 # переменной count_consonants в си присваиваем 0
...
.L2:
	...
	mov	QWORD PTR -40[rbp], rax # на стек по адресу rbp[-40] записали размер строки в файле
	mov	rax, QWORD PTR -32[rbp]
	mov	rdi, rax
...
.L4:
	...
	cmp	rax, QWORD PTR -40[rbp] # сравниваем i с размером строки (переменная - file_size в си)
...
.L9:
	...
	add	DWORD PTR -4[rbp], 1 # увеличение кол-ва гласных на 1
.L7:
	mov	rdx, QWORD PTR -24[rbp] 
	mov	rax, QWORD PTR -48[rbp]
	add	rax, rdx # перемещение указателя в памяти
	...
	add	DWORD PTR -8[rbp], 1 # увеличили кол-во согласных на 1
.L8:
	add	QWORD PTR -24[rbp], 1 # прибавляем к счётчику 1
.L6:
	mov	rax, QWORD PTR -24[rbp]
	cmp	rax, QWORD PTR -40[rbp] # сравнение счётчика цикла с кол-вом элементов в файле
	...
	mov	edx, DWORD PTR -4[rbp] # через edx передаём кол-во гласных
	...
	mov	edx, DWORD PTR -8[rbp] # через edx передаём кол-во согласных
	mov	rax, QWORD PTR -56[rbp] 
	lea	rcx, .LC5[rip]
	mov	rsi, rcx # через rsi передаём строку, означает вывод данных в файл 
